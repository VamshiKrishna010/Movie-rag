...
build_chunks(conn)               SELECT movie + joined genres/cast/crew/keywords
  ...
  ▼
ChunkRecord(movie_id, chunk_type, content)
                                  multiple chunks per movie: full, plot, themes
```
> Landmine: editing a movie's text does **not** re-embed automatically — rerun `run_embed.py`. If only theme rules changed, run `run_embed.py --refresh-types themes`.

### A3. Embed + index — text → vector + FTS  (`scripts/run_embed.py`, `app/ingest/embedder.py`)

```
embed_and_store()
  │  build_chunks() / build_chunks_missing()      get ChunkRecord rows
  ▼
embed_texts(texts, batch_size=256)                SentenceTransformer
...
`--refresh-types themes` deletes and rebuilds only theme chunks.
  ...
  user question  ───►  │  POST /query   {question, k, include_chunks}  │
        ...
        embed_query_async(question)  ─────┤  "query:"-prefixed text -> 384-dim vector
                  ...
                  if empty ──►  raise HTTPException 404                      (guard in
                   ...
                   └─────────────────────────────────────────┘
                                          │
                   AsyncOpenAI client → Groq  (llama-3.3-70b-versatile)
...
| **B3 Generate** | `generator.generate` | Groq LLM answers **only** from that context, low temperature (0.3), citing title+year. |
...
**Online:** embed the question → single-SQL dense + FTS search fused by RRF → top-k → Groq answers strictly grounded in those chunks → return answer + sources.
