...
2. **LLM calls.** The slowest part of a `/query` request is the Cerebras HTTP call; `await` lets other requests progress during that wait.
...
- **`chunks`** — the retrieval table. **One row per movie** (`chunk_type = 'full'`). Columns:
...
Cerebras runs an OpenAI-compatible API; the SDK is the standard `openai` Python client. The LLM tokenizes the prompt server-side using `gpt-oss-120b`'s own tokenizer — *not* tiktoken, not BGE's WordPiece. The model predicts the next token repeatedly until `max_tokens` or a stop token.
...
| LLM generation | `gpt-oss-120b`'s own tokenizer | Server-side at Cerebras |
...
2. **One chunk per movie** today (`chunk_type = 'full'`). Manual movie text edits do *not* re-embed — re-run `scripts/run_embed.py` after changes.
